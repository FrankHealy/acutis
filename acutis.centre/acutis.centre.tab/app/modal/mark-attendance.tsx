  ...
  return (
    <View style={styles.screen}>
      <View style={styles.card}>
        <Text style={styles.title}>{t("dashboard.rollCall", "Roll Call")}</Text>
        <Text style={styles.subtitle}>{t("rollCall.modalSubtitle", "Open the unit roll call screen to capture attendance offline.")}</Text>
        <Pressable style={styles.button} onPress={() => router.replace("/(tabs)/roll-call/detox")}>
          <Text style={styles.buttonText}>{t("dashboard.rollCall", "Roll Call")}</Text>
        </Pressable>
      </View>
    </View>
  );
