...
public sealed class AcutisAmbulatoryDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<VideoConsultation>(entity =>
        {
            ...
            entity.HasKey(x => x.Id);
            entity.Property(x => x.RoomName).HasMaxLength(64);
            entity.Property(x => x.Status).HasConversion<string>().HasMaxLength(24);
            entity.HasIndex(x => x.AppointmentId).IsUnique();
            entity.HasIndex(x => x.RoomName).IsUnique();
            entity.HasOne(x => x.Appointment).WithOne(x => x.VideoConsultation)
                .HasForeignKey<VideoConsultation>(x => x.AppointmentId).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        modelBuilder.Entity<VideoConsultationInvitation>(entity =>
        {
            ...
            entity.HasKey(x => x.Id);
            entity.Property(x => x.TokenHash).HasMaxLength(32).IsFixedLength();
            entity.HasIndex(x => x.TokenHash).IsUnique();
            entity.HasOne(x => x.VideoConsultation).WithMany(x => x.Invitations)
                .HasForeignKey(x => x.VideoConsultationId).OnDelete(DeleteBehavior.Cascade);
        });
    }
}
