...
const nav: [View, string, typeof LayoutDashboard, string][] = [
  ...
  ["clients", "nav.clients", Users, "/clients"],
  ...
  ["notes", "nav.notes.practitioner", FileText, "/notes"],
  ["forms", "nav.forms", Activity, "/forms"],
  ["settings", "nav.configuration", Settings, "/settings"],
];
const localeStorageKey = "acutis.practitioner.locale";
...
function Content({ view }: { view: View }) {
  ...
  const [dialog, setDialog] = useState<"client" | "appointment" | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!a.ok) throw new Error("Unable to verify Practitioner membership.");
      const m = (await a.json()) as { tenantId: string }[];
      if (!m[0]) throw new Error("No active Practitioner membership.");
      ...
      if (!r.ok)
        throw new Error(
          r.status === 403
            ? "Practitioner access denied."
            : "Unable to load the practice.",
        );
  ...
  if (busy)
    return <State locale={locale} theme={theme} title="Acutis Practitioner" text={`${productText(locale, "common.loading")}…`} />;
  if (error || !data)
    return (
      <State
        locale={locale}
  ...
  const join = async (item: ScheduleItem) => {
    ...
    try {
      ...
      try {
        // These calls invoke getUserMedia. The browser owns the permission UI;
        // the consultation is not considered connected until both tracks exist.
    ...
    >
      <header className="brand">
        ...
        <div>
          ...
          <span>{productText(locale, "product.practitioner")}</span>
        </div>
        <div className="brand-context">
          ...
          <ShieldCheck />
          <span>{roles.join(" · ") || productText(locale, "practitioner.role")}</span>
          <b>{productText(locale, data.scope === "practice" ? "practitioner.practice_scope" : "practitioner.caseload_scope")}</b>
        </div>
      </header>
      ...
      <main className="page">
        <div className="page-heading">
          <div>
            <span className="eyebrow">{productText(locale, "practitioner.eyebrow")}</span>
            <h1>{productText(locale, nav.find((x) => x[0] === activeView)?.[1] ?? "nav.dashboard")}</h1>
            <p>{productText(locale, "practitioner.description")}</p>
          </div>
        ...
        {activeView === "appointments" && (
          <ClinicalCalendar
            ...
            onOpen={(x) => setSelectedAppointment(data.appointments.find((appointment) => appointment.id === x.id) ?? null)}
          />
        )}{" "}
        {activeView === "clients" && (
          <Clients data={data} onAdd={() => setDialog("client")} />
        )}{" "}
        {activeView === "assessments" && (
          <History
            ...
            items={data.assessments.map((x) => ({
              id: x.id,
              name:
                data.clients.find((c) => c.id === x.clientId)?.displayName ??
                "Client",
        ...
        {activeView === "forms" && (
          <Panel title="Clinical forms">
            <p>
              Practitioner-owned definitions, assignments and responses. Forms
              remain isolated in the Practitioner database.
            </p>
          </Panel>
        )}{" "}
        {activeView === "settings" && (
        ...
        {dialog && (
          <Modal
            title={dialog === "client" ? "Add client" : "New appointment"}
            close={() => setDialog(null)}
          >
            {dialog === "client" ? (
              <ClientForm send={send} />
            ) : (
              <AppointmentForm clients={data.clients} send={send} />
            )}
          </Modal>
        )}{" "}
        {selectedAppointment && (
          <Modal title={selectedAppointment.title} close={() => setSelectedAppointment(null)}>
            <AppointmentActions appointment={selectedAppointment} tenant={tenant} headers={headers} locale={locale} onJoin={() => { setSelectedAppointment(null); void join(selectedAppointment); }} />
          </Modal>
        )}{" "}
        {room && (
          <VideoConsultation
        ...
        {callError && !room && (
          <div className="callbar" role="alert">
            <Video />
            <strong>Unable to start consultation</strong>
            <span>{callError}</span>
...
function AppointmentActions({ appointment, tenant, headers, locale, onJoin }: { appointment: Appointment; tenant: string; headers: Record<string, string>; locale: string; onJoin: () => void }) {
  const [email, setEmail] = useState(appointment.client?.email ?? "");
  const [invitation, setInvitation] = useState<{ id: string; sendStatus: string } | null>(null);
  const [status, setStatus] = useState("");
  const sendInvitation = async () => {
    setStatus("Sending invitation…");
    const path = invitation ? `/api/tenants/${tenant}/video-invitations/${invitation.id}/resend` : `/api/tenants/${tenant}/appointments/${appointment.id}/video-invitations`;
    const response = await fetch(`${api}${path}`, { method: "POST", headers, body: JSON.stringify({ recipientEmail: email, locale }) });
    const result = await response.json() as { id?: string; sendStatus?: string; sanitisedSendFailureReason?: string; message?: string };
    if (!response.ok || !result.id) { setStatus(result.message ?? "Unable to send the invitation."); return; }
    setInvitation({ id: result.id, sendStatus: result.sendStatus ?? "Pending" });
    setStatus(result.sendStatus === "Sent" ? `Invitation sent to ${email}.` : `Invitation saved. ${result.sanitisedSendFailureReason ?? "Email delivery needs attention."}`);
  };
  const revoke = async () => {
    if (!invitation) return;
    const response = await fetch(`${api}/api/tenants/${tenant}/video-invitations/${invitation.id}/revoke`, { method: "POST", headers });
    setStatus(response.ok ? "Invitation revoked." : "Unable to revoke the invitation.");
    if (response.ok) setInvitation(null);
  };
  return <div className="settings-form"><p><strong>{appointment.client?.displayName ?? "Client"}</strong><br />{new Date(appointment.startsAtUtc).toLocaleString(locale, { dateStyle: "full", timeStyle: "short" })}</p>{appointment.deliveryMode === "video" ? <><label>Recipient email<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label><div className="capture-actions"><button className="primary" disabled={!email} onClick={() => void sendInvitation()}>{invitation ? "Resend and replace" : "Email invitation"}</button>{invitation && <button onClick={() => void revoke()}>Revoke</button>}<button onClick={onJoin}>Start consultation</button></div>{status && <p role="status">{status}</p>}</> : <p>This is an in-person appointment.</p>}</div>;
}
...
function VideoConsultation({
  ...
  useEffect(() => {
    ...
    const attachLocalCamera = () => {
      ...
      if (publication?.track && localVideoElement) {
    ...
    return () => {
      ...
      room.localParticipant.videoTrackPublications.forEach((publication) => {
        if (publication.track && localVideoElement) {
          publication.track.detach(localVideoElement);
        }
      });
      room.remoteParticipants.forEach((participant) => {
        participant.trackPublications.forEach((publication) =>
  ...
  const toggleCamera = async () => {
    ...
    if (enabled) {
      ...
      if (publication?.track && localVideo.current) {
        publication.track.attach(localVideo.current);
      }
    }
  };
  ...
  const createInvitation = async () => {
    if (!appointment) return;
    const response = await fetch(`${api}/api/tenants/${tenant}/appointments/${appointment.id}/video-invitations`, { method: "POST", headers, body: JSON.stringify({ recipientEmail: appointment.client?.email, locale: document.documentElement.lang }) });
    const result = await response.json() as { id?: string; sendStatus?: string; sanitisedSendFailureReason?: string; message?: string };
    if (!response.ok) { setSaveStatus(result.message ?? "Unable to create invitation"); return; }
    setInvitationId(result.id ?? ""); setSaveStatus(result.sendStatus === "Sent" ? `Invitation emailed to ${appointment.client?.email}` : result.sanitisedSendFailureReason ?? "Invitation saved; email delivery failed");
  };
  ...
  return (
    <div className="call-backdrop" role="dialog" aria-modal="true" aria-label={title}>
      <section className="call-window">
        ...
        </div><aside className="consultation-capture"><h3>Consultation record</h3><label>Session notes<textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Themes, interventions and agreed actions" /></label><label>Observations<textarea value={observations} onChange={(event) => setObservations(event.target.value)} placeholder="Presentation, engagement, mood and risk observations" /></label><div className="capture-actions"><button className="call-control" onClick={() => void saveCapture()}>Save record</button><button className="call-control" onClick={() => void createInvitation()}>{invitationId ? "Send replacement invitation" : "Email invitation"}</button></div>{saveStatus && <small>{saveStatus}</small>}</aside></div>
        <footer>
          ...
          <button className="call-leave" onClick={onLeave}>Leave consultation</button>
        </footer>
      </section>
    </div>
  );
...
function Dashboard({
  ...
  data: Data;
  setDialog: (x: "client" | "appointment") => void;
}) {
    ...
    <>
      <section className="metrics">
        <Metric
          ...
          value={data.clients.filter((x) => x.status === "active").length}
          note={`${data.staff.length} practice staff`}
          icon={<Users />}
        />
        <Metric
          ...
          note="Across the practice"
          icon={<CalendarDays />}
        />
        <Metric
          label="Care plans"
          value={data.carePlans.filter((x) => x.status === "active").length}
          note="Active and under review"
          icon={<HeartHandshake />}
        />
        <Metric
      ...
      <section className="quick">
        ...
        <div>
          <button onClick={() => setDialog("client")}>
            <Users />
            <b>Add client</b>
            <span>Create a practice client record</span>
          </button>
          ...
          <a href="/assessments">
            <ClipboardCheck />
            <b>Assessment</b>
            <span>Record clinical capture</span>
          </a>
          <a href="/care-plans">
            <HeartHandshake />
            <b>Care plan</b>
            <span>Review goals and actions</span>
          </a>
        </div>
      </section>
      <div className="dashboard-grid">
        <Panel title="Practice timeline">
            ...
            {upcoming.map((x) => (
              <article key={x.id}>
                ...
                <div>
                  <b>{x.title}</b>
                  <span>
                    {x.client?.displayName ?? "Practice event"} ·{" "}
                    {x.deliveryMode}
                  </span>
                </div>
              </article>
        ...
        <Panel title="Clinical programmes">
...
function Clients({ data, onAdd }: { data: Data; onAdd: () => void }) {
  const [query, setQuery] = useState("");
  const clients = data.clients.filter((client) => `${client.displayName} ${client.preferredName ?? ""} ${client.email ?? ""}`.toLowerCase().includes(query.toLowerCase()));
  return (
    <Panel title="Practice clients">
      <div className="table-head">
        <p>{data.clients.length} active client records</p>
        <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search clients" aria-label="Search clients" />
      ...
      <div className="data-table">
        ...
        {clients.map((x) => (
          <div className="tr" key={x.id}>
            ...
            <span>{x.practitionerDisplayName}</span>
            <span>
              <em>{x.status}</em>
            </span>
          </div>
        ))}
        {clients.length === 0 && <div className="empty-directory"><Users /><strong>No clients match this view</strong><span>Clear the search or add a client to the practice.</span></div>}
      </div>
    </Panel>
  );
}
...
function SettingsView({
  ...
  const [settings, setSettings] = useState(() => {
    const defaults = { dayStart: "08:00", dayEnd: "18:00", appointmentMinutes: "60", videoInvitations: true, observationPrompt: true };
    try { return { ...defaults, ...JSON.parse(localStorage.getItem("acutis.practitioner.settings") ?? "{}") as object }; } catch { return defaults; }
  });
  const save = () => { localStorage.setItem("acutis.practitioner.settings", JSON.stringify(settings)); setSaved(true); };
  return (
    <div className="settings-layout">
      <Panel title={leadership ? "Practice configuration" : "My working preferences"}>
        <div className="settings-form"><label>Working day starts<input type="time" value={settings.dayStart} onChange={(event) => setSettings({ ...settings, dayStart: event.target.value })} /></label><label>Working day ends<input type="time" value={settings.dayEnd} onChange={(event) => setSettings({ ...settings, dayEnd: event.target.value })} /></label><label>Default appointment length<select value={settings.appointmentMinutes} onChange={(event) => setSettings({ ...settings, appointmentMinutes: event.target.value })}><option value="30">30 minutes</option><option value="45">45 minutes</option><option value="60">60 minutes</option><option value="90">90 minutes</option></select></label><label className="check"><input type="checkbox" checked={settings.videoInvitations} onChange={(event) => setSettings({ ...settings, videoInvitations: event.target.checked })} />Generate guest links for video appointments</label><label className="check"><input type="checkbox" checked={settings.observationPrompt} onChange={(event) => setSettings({ ...settings, observationPrompt: event.target.checked })} />Show observations during consultation</label><button className="primary" onClick={save}>Save configuration</button>{saved && <small>Configuration saved on this device.</small>}</div>
      </Panel>
      <Panel title="My access">
        <p>{roles.join(", ") || "Practitioner"}</p>
        <p>
          Role-aware access is enforced by the Practitioner API and Keycloak
          membership.
        </p>
      </Panel>
      <Panel title="Clinical defaults"><p>CBT, DBT and Gambling Recovery are available for programme scheduling.</p><p>Video consultations create expiring guest invitations and keep recording disabled.</p></Panel>
      <Panel title="Practice team">
        <div className="record-list">
          {staff.map((x) => (
