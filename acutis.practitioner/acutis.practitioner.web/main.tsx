...
const pathToView = { "/clients": "clients", "/appointments": "appointments", "/assessments": "assessments", "/care-plans": "care-plans", "/notes": "notes", "/forms": "forms", "/settings": "settings" } as const;
function App() {
  ...
  const joinMatch = pathname.match(/^\/meetings\/join\/([^/]+)$/);
  const view = pathToView[pathname as keyof typeof pathToView] ?? "dashboard";
  return joinMatch ? <GuestJoin productName="Acutis Practitioner" invitationToken={decodeURIComponent(joinMatch[1])} /> : <ProductSpaAuthProvider issuer={import.meta.env.VITE_KEYCLOAK_ISSUER} clientId={import.meta.env.VITE_KEYCLOAK_CLIENT_ID}><Workspace view={view} /></ProductSpaAuthProvider>;
}
