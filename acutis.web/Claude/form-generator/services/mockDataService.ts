 ...
 * Reads/writes to JSON files in dat/mock directory.
 * 
 * Usage:
 * - Set NEXT_PUBLIC_USE_MOCK_DATA=true to enable
...
const MOCK_DATA_DIR = path.join(process.cwd(), 'dat', 'mock');
...
export class MockAdmissionsService {
  ...
  getStats(date?: string): AdmissionsStats {
    ...
    const expectedToday = todayAdmissions.filter(a => 
      a.status === 'expected'
    ).length;
    
    const completedToday = todayAdmissions.filter(a => 
      a.status === 'completed'
    ).length;
    
    const needsReview = data.admissions.filter(a => 
      a.status === 'in_progress' || (a.status === 'expected' && !a.phoneEvalCompleted)
    ).length;
    ...
    return {
      ...
      completedToday,
      needsReview,
      occupancy: data.stats.occupancy, // Keep occupancy from file
    };
  }
  ...
  startSession(admissionId: string): AdmissionSession {
    ...
    const existing = sessionsData.sessions.find(s => 
      s.admissionId === admissionId && !s.completedAt
    );
  ...
  getSession(admissionId: string, sessionId: string): AdmissionSession | null {
    ...
    return data.sessions.find(s => 
      s.id === sessionId && s.admissionId === admissionId
    ) || null;
  }
  ...
  updateSession(
    admissionId: string,
    sessionId: string,
    updates: { currentStep: number; data: any }
  ): AdmissionSession {
    const sessionsData = readJsonFile<SessionsData>('sessions.json');
    
    const index = sessionsData.sessions.findIndex(s => 
      s.id === sessionId && s.admissionId === admissionId
    );
  ...
  reset(): void {
    // This would reset the files to their initial state
    // Implement if needed
  }
}
