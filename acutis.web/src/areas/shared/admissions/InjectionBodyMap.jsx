...
export default function InjectionBodyMap({
  value,
  onChange,
  width = 320,
  height = "auto",
  ...
  const [internalValue, setInternalValue] = useState(value ?? { marks: [] });

  const isControlled = value !== undefined;
  const marks = (isControlled ? value : internalValue)?.marks ?? [];

  const patternId = useMemo(() => `grid-${internalId.replace(/:/g, "")}`, [internalId]);

  const emitChange = (nextMarks) => {
    const nextValue = { marks: nextMarks };
  ...
  const handleSvgClick = (event) => {
    ...
    emitChange([...marks, createMark(point.x, point.y)]);
  };

  const removeMark = (id) => {
    if (disabled) return;
    emitChange(marks.filter((mark) => mark.id !== id));
  };

  const clearMarks = () => {
    if (disabled || marks.length === 0) return;
    emitChange([]);
  };
    ...
    <div className={className}>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            background: disabled || marks.length === 0 ? "#f3f4f6" : "#ffffff",
            cursor: disabled || marks.length === 0 ? "not-allowed" : "pointer",
          }}
        >
          {clearButtonLabel}
        </button>
      ...
      <div
        style={{
          ...
          padding: 12,
          background: "#ffffff",
          maxWidth: typeof width === "number" ? width + 32 : width,
        }}
      >
        <svg
          ref={svgRef}
        ...
        >
          ...
          <rect x="0" y="0" width={VIEWBOX_WIDTH} height={VIEWBOX_HEIGHT} fill={`url(#${patternId})`} />
          ...
          {marks.map((mark, index) => (
            <g key={mark.id}>
              ...
              >
                <title>{`${markLabelPrefix} ${index + 1}: x ${mark.x}, y ${mark.y}`}</title>
              </circle>
              <text
                x={mark.x + 3.2}
                y={mark.y - 3.2}
          ...
          {hoverPoint && showCoordinates && !disabled && (
            <g pointerEvents="none">
              ...
              />
              <rect
                x={Math.min(hoverPoint.x + 4, VIEWBOX_WIDTH - 42)}
                y={Math.max(hoverPoint.y - 13, 2)}
                width="40"
                height="10"
                rx="2"
                fill="#111827"
              ...
              />
              <text
                x={Math.min(hoverPoint.x + 6, VIEWBOX_WIDTH - 40)}
                y={Math.max(hoverPoint.y - 6, 8)}
                fontSize="4"
                fontFamily="Arial, sans-serif"
                fill="#ffffff"
              >
                {`${hoverPoint.x.toFixed(1)}, ${hoverPoint.y.toFixed(1)}`}
              </text>
            </g>
          )}
        </svg>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          >
            {marks.map((mark, index) => (
              ...
              >
                <div style={{ fontWeight: 600, marginBottom: 4 }}>{markLabelPrefix} {index + 1}</div>
                <div>x: {mark.x}</div>
                <div>y: {mark.y}</div>
              </div>
            ))}
