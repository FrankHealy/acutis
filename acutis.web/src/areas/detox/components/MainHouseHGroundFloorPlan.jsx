...
const TEMP_ADMIN_UPPER_BLOCK_GAP = 40;
const TEMP_ADMIN_SIDE_RECT_WIDTH = 700;
const TEMP_ADMIN_UPPER_RECT_HEIGHT = 39;
const TEMP_ADMIN_UPPER_RECT_BASE_OFFSET = 100;
const TEMP_CLOCK_TOWER_DOOR_HEIGHT = 16;
const TEMP_CLOCK_TOWER_STAIR_WIDTH = 31;
const TEMP_CLOCK_TOWER_STAIR_HEIGHT = 17;
const TEMP_DRUG_UNIT_ROOM_WIDTH = 60;
const TEMP_DRUG_UNIT_ROOM_HEIGHT = 37;
const TEMP_DONT_KNOW_LINK_HEIGHT = 30;
...
const TEMP_DRUG_UNIT_ROOM_DOOR_WIDTH = 14;
const TEMP_ADMIN_MEDICAL_WING_DOOR_WIDTH = 18;
...
export default function MainHouseHGroundFloorPlan() {
  ...
  const tempAdminSideRectTop = gridTop(9) - 280;
  const tempAdminUpperBlockLeft = tempTopHorizontalLeft;
  const tempAdminUpperBlockBottom =
    tempTopHorizontalTop - TEMP_ADMIN_UPPER_BLOCK_GAP;
  const tempAdminUpperBlockTop = tempAdminSideRectTop;
  const tempAdminUpperBlockHeight = tempAdminUpperBlockBottom - tempAdminUpperBlockTop;
  const tempElcInternalWallTop = gridTop(4);
  const tempElcInternalVerticalLeft =
    tempAdminUpperBlockLeft +
    Math.round((TEMP_TOP_HORIZONTAL.width - TEMP_INTERNAL_WALL_THICKNESS) / 2);
  const tempElcInternalVerticalTop = tempAdminUpperBlockTop + TEMP_WALL_THICKNESS;
  const tempElcInternalVerticalHeight =
    tempElcInternalWallTop - tempElcInternalVerticalTop;
  const tempElcWingLabelLeft = tempAdminUpperBlockLeft + 8;
  const tempElcWingLabelTop = tempAdminUpperBlockTop + 8;
  const tempAdminSideRectLeft = tempAdminUpperBlockLeft + TEMP_TOP_HORIZONTAL.width - 4;
  const tempAdminSideRectHeight = TEMP_TOP_HORIZONTAL.width;
  const tempElcDrugJoinLeft = tempAdminUpperBlockLeft + TEMP_TOP_HORIZONTAL.width - TEMP_WALL_THICKNESS;
  const tempElcDrugJoinTop = tempAdminSideRectTop;
  const tempElcDrugJoinHeight = Math.min(tempAdminUpperBlockHeight, tempAdminSideRectHeight);
  const tempDrugUnitWingLabelLeft = tempAdminSideRectLeft + 8;
  const tempDrugUnitWingLabelTop = tempAdminSideRectTop + 8;
  const tempDrugUnitRoomLeft =
    tempAdminSideRectLeft + TEMP_ADMIN_SIDE_RECT_WIDTH - TEMP_DRUG_UNIT_ROOM_WIDTH - 6;
  const tempDrugUnitRoomTop = tempAdminSideRectTop + 6;
  const tempDontKnowLinkLeft = gridLeft(26) + 60;
  const tempDontKnowLinkTop = tempDrugUnitRoomTop;
  const tempDontKnowLinkWidth = tempDrugUnitRoomLeft - tempDontKnowLinkLeft;
  const tempDrugUnitInnerSpanWidth =
    tempDontKnowLinkLeft - (tempAdminSideRectLeft + TEMP_WALL_THICKNESS);
  const tempDrugUnitThirdWidth = Math.round(tempDrugUnitInnerSpanWidth / 3);
  const tempDrugUnitDivider1Left =
    tempAdminSideRectLeft + TEMP_WALL_THICKNESS + tempDrugUnitThirdWidth;
  const tempDrugUnitDivider2Left =
    tempAdminSideRectLeft + TEMP_WALL_THICKNESS + tempDrugUnitThirdWidth * 2;
  const tempDrugUnitDividerTop = tempAdminSideRectTop + TEMP_WALL_THICKNESS;
  const tempDrugUnitDividerHeight = tempAdminSideRectHeight - TEMP_WALL_THICKNESS * 2;
  const tempDuBottomLowerRectLeft = tempDontKnowLinkLeft + 386;
  const tempDuBottomLowerRectTop = tempDontKnowLinkTop + TEMP_DONT_KNOW_LINK_HEIGHT - 1;
  const TEMP_DU_BOTTOM_LOWER_RECT_WIDTH = 50;
  const TEMP_DU_BOTTOM_LOWER_RECT_HEIGHT = 50;
  const tempDuBottomCooridorLabelLeft = tempDontKnowLinkLeft + 8;
  const tempDuBottomCooridorLabelTop = tempDontKnowLinkTop + 8;
  const TEMP_DU_BOTTOM_STAIR_GAP_WIDTH = 22;
  const TEMP_DU_BOTTOM_NIGHT_MAN_DOOR_WIDTH = 18;
  const tempDuBottomNightManDoorLeft =
    tempDuBottomLowerRectLeft +
    Math.round((TEMP_DU_BOTTOM_LOWER_RECT_WIDTH - TEMP_DU_BOTTOM_NIGHT_MAN_DOOR_WIDTH) / 2);
  const TEMP_DU_LOWER_ROOMS_SMALL_WIDTH = 26;
  const TEMP_DU_LOWER_ROOMS_LARGE_WIDTH = 38;
  const TEMP_DU_LOWER_ROOM3_WIDTH = 148;
  const tempDontKnowLabelLeft = tempDrugUnitRoomLeft + 8;
  const tempDontKnowLabelTop = tempDrugUnitRoomTop + 8;
  const tempDrugUnitRoomDoorTop = tempDrugUnitRoomTop + 4;
  const tempDrugUnitRoomMidLineTop =
    tempDrugUnitRoomTop + Math.round(TEMP_DRUG_UNIT_ROOM_HEIGHT / 2);
  const tempDrugUnitRoomMidLineLeft =
  ...
  const tempDrugUnitWingStairTop =
    tempAdminSideRectTop + tempAdminSideRectHeight - TEMP_CLOCK_TOWER_STAIR_HEIGHT - 6;
  const tempDrugUnitWingStairAreaLeft = tempDuBottomLowerRectLeft + TEMP_INTERNAL_WALL_THICKNESS;
  const tempDrugUnitWingStairAreaTop =
    tempDontKnowLinkTop + TEMP_DONT_KNOW_LINK_HEIGHT + TEMP_INTERNAL_WALL_THICKNESS;
  const tempDrugUnitWingStairAreaWidth =
    tempAdminSideRectLeft +
    TEMP_ADMIN_SIDE_RECT_WIDTH -
    TEMP_WALL_THICKNESS -
    tempDrugUnitWingStairAreaLeft;
  const tempDrugUnitWingStairAreaHeight =
    tempAdminSideRectTop +
    tempAdminSideRectHeight -
    TEMP_WALL_THICKNESS -
    tempDrugUnitWingStairAreaTop;
  const tempDuFrontStairwellLabelLeft = tempDrugUnitWingStairAreaLeft + 8;
  const tempDuFrontStairwellLabelTop = tempDrugUnitWingStairAreaTop + 8;
  const tempOverDrugsUnitStaticaseLabelLeft = tempDrugUnitWingStairLeft - 8;
  const tempOverDrugsUnitStaticaseLabelTop = tempDrugUnitWingStairTop - 24;
  ...
  const tempAdminMedicalWingInnerWallTop = gridTop(11) + 20;
  ...
  const tempDuBottomLowerRectRight =
    tempDuBottomLowerRectLeft + TEMP_DU_BOTTOM_LOWER_RECT_WIDTH - TEMP_INTERNAL_WALL_THICKNESS;
  const tempDuBottomLowerRectBottom = tempBottomHorizontalTop + TEMP_INTERNAL_WALL_THICKNESS;
  const tempDuBottomLowerRectLeftWallHeight =
    tempAdminSideRectTop + tempAdminSideRectHeight - tempDuBottomLowerRectTop;
  const tempDuLowerRoomsTop = tempDontKnowLinkTop + TEMP_DONT_KNOW_LINK_HEIGHT;
  const tempDuLowerRoomsBottom =
    tempAdminSideRectTop + tempAdminSideRectHeight - TEMP_WALL_THICKNESS;
  const tempDuLowerRoomsHeight = tempDuLowerRoomsBottom - tempDuLowerRoomsTop;
  const tempDuLowerRoom1Left = tempDontKnowLinkLeft;
  const TEMP_DU_LOWER_ROOM1_WIDTH = TEMP_DU_LOWER_ROOMS_SMALL_WIDTH + 50;
  const tempDuLowerRoom4Left =
    tempDuLowerRoom1Left + TEMP_DU_LOWER_ROOMS_SMALL_WIDTH * 2 + TEMP_DU_LOWER_ROOMS_LARGE_WIDTH + 210;
  const tempDuLowerRoom3Left = tempDuLowerRoom4Left - TEMP_DU_LOWER_ROOM3_WIDTH;
  const tempDuLowerRoom2Left = tempDuLowerRoom1Left + TEMP_DU_LOWER_ROOM1_WIDTH;
  const tempDuLowerRoom2Width = tempDuLowerRoom3Left - tempDuLowerRoom2Left;
  const tempDuLowerRoom4Width =
    tempDuBottomLowerRectLeft - TEMP_INTERNAL_WALL_THICKNESS - tempDuLowerRoom4Left;
  const tempCouncillorsOfficeLabelLeft = tempDuLowerRoom4Left + 8;
  const tempCouncillorsOfficeLabelTop = tempDuLowerRoomsTop + 8;
  const tempDuBottomLowerRectLineStartLeft = tempDuBottomLowerRectRight + 1;
  const tempDuBottomLowerRectLineStartTop =
    tempDuBottomLowerRectTop + TEMP_DU_BOTTOM_LOWER_RECT_HEIGHT;
  const tempDuBottomLowerRectLineEndLeft = tempDuBottomLowerRectLeft + 1;
  const tempDuBottomLowerRectLineEndTop =
    tempAdminSideRectTop + tempAdminSideRectHeight - 16;
  const tempDuBottomLowerRectLineWidth =
    tempDuBottomLowerRectLineEndLeft - tempDuBottomLowerRectLineStartLeft;
  const tempDuBottomLowerRectLineRise =
    tempDuBottomLowerRectLineEndTop - tempDuBottomLowerRectLineStartTop;
  const tempDuBottomLowerRectLineLength = Math.hypot(
    tempDuBottomLowerRectLineWidth,
    tempDuBottomLowerRectLineRise,
  );
  const tempDuBottomLowerRectLineAngle = Math.atan2(
    tempDuBottomLowerRectLineRise,
    tempDuBottomLowerRectLineWidth,
  );
  const tempDuBottomLowerFillLeft = tempDuBottomLowerRectLeft + TEMP_INTERNAL_WALL_THICKNESS;
  const tempDuBottomLowerFillTop = tempDuBottomLowerRectTop + TEMP_INTERNAL_WALL_THICKNESS - 1;
  const tempDuBottomLowerFillWidth =
    TEMP_DU_BOTTOM_LOWER_RECT_WIDTH - TEMP_INTERNAL_WALL_THICKNESS * 2;
  const tempDuBottomLowerFillHeight =
    tempDuBottomLowerRectLineEndTop - tempDuBottomLowerFillTop - TEMP_INTERNAL_WALL_THICKNESS;
  const tempAdminMedicalWingLowerFillLeft = tempTopHorizontalLeft + TEMP_WALL_THICKNESS;
  const tempAdminMedicalWingLowerFillTop =
    tempAdminMedicalWingInnerWallTop + TEMP_INTERNAL_WALL_THICKNESS;
  const tempAdminMedicalWingLowerFillWidth = TEMP_TOP_HORIZONTAL.width - TEMP_WALL_THICKNESS * 2;
  const tempAdminMedicalWingLowerFillHeight =
    tempBottomHorizontalTop - tempAdminMedicalWingLowerFillTop;
  const tempAdminMedicalWingUpperWallTop = tempAdminMedicalWingInnerWallTop - 60;
  const tempAdminMedicalWingTopFillLeft = tempAdminMedicalWingLowerFillLeft;
  const tempAdminMedicalWingTopFillTop = tempTopHorizontalTop + TEMP_WALL_THICKNESS;
  const tempAdminMedicalWingTopFillWidth = tempAdminMedicalWingLowerFillWidth;
  const tempAdminMedicalWingTopFillHeight =
    tempAdminMedicalWingUpperWallTop - tempAdminMedicalWingTopFillTop;
  const tempAdminMedicalWingUpperFillLeft = tempAdminMedicalWingLowerFillLeft;
  const tempAdminMedicalWingUpperFillTop = tempAdminMedicalWingUpperWallTop + TEMP_INTERNAL_WALL_THICKNESS;
  const tempAdminMedicalWingUpperFillWidth = tempAdminMedicalWingLowerFillWidth;
  const tempAdminMedicalWingUpperFillHeight =
    tempAdminMedicalWingInnerWallTop - tempAdminMedicalWingUpperFillTop;
  const tempDoctorwWaitingRoomLabelLeft = tempAdminMedicalWingLowerFillLeft + 8;
  const tempDoctorwWaitingRoomLabelTop = tempAdminMedicalWingLowerFillTop + 8;
  const tempSurgeryStoreLabelLeft = tempAdminMedicalWingLowerFillLeft + 8;
  const tempSurgeryStoreLabelTop = tempAdminMedicalWingUpperWallTop + 8;
  const tempAdminMedicalWingDoorLeft =
    tempTopHorizontalLeft +
    TEMP_WALL_THICKNESS +
    Math.round(
      (TEMP_TOP_HORIZONTAL.width - TEMP_WALL_THICKNESS * 2 - TEMP_ADMIN_MEDICAL_WING_DOOR_WIDTH) / 2
    );
        ...
        >
          ...
          >
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminSideRectLeft,
                top: tempAdminSideRectTop,
                width: TEMP_ADMIN_SIDE_RECT_WIDTH,
                height: tempAdminSideRectHeight,
                background: "#D9D9D9",
                zIndex: 54,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitWingLabelLeft,
                top: tempDrugUnitWingLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminSideRectLeft + TEMP_ADMIN_SIDE_RECT_WIDTH - TEMP_WALL_THICKNESS,
                top: tempAdminSideRectTop,
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitDivider1Left,
                top: tempDrugUnitDividerTop,
                width: TEMP_INTERNAL_WALL_THICKNESS,
                height: tempDrugUnitDividerHeight,
                background: "#333",
                zIndex: 55,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitWingStairAreaLeft,
                  top: tempDrugUnitWingStairAreaTop,
                  width: tempDrugUnitWingStairAreaWidth,
                  height: tempDrugUnitWingStairAreaHeight,
                  background: "#FDE68A",
                  zIndex: 54,
                  pointerEvents: "none",
                }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempOverDrugsUnitStaticaseLabelLeft,
                top: tempOverDrugsUnitStaticaseLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
            ...
            >
              Over Drugs Unit Staticase
            </div>
            <div
              style={{
                position: "absolute",
                left: tempDuFrontStairwellLabelLeft,
                top: tempDuFrontStairwellLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
            ...
            />
              <div
                style={{
                  position: "absolute",
                  left: tempDontKnowLinkLeft,
                top: tempDontKnowLinkTop,
                width: tempDontKnowLinkWidth,
                height: TEMP_DONT_KNOW_LINK_HEIGHT,
                background: "#FDE68A",
                zIndex: 54,
                pointerEvents: "none",
              }}
            />
              <div
                style={{
                  position: "absolute",
                  left: tempDuBottomCooridorLabelLeft,
                top: tempDuBottomCooridorLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempCouncillorsOfficeLabelLeft,
                  top: tempCouncillorsOfficeLabelTop,
                  padding: "3px 8px",
                  background: "rgba(255,255,255,0.96)",
                  border: "1px solid rgba(239,68,68,0.45)",
                  borderRadius: 6,
            ...
            <div
              style={{
                position: "absolute",
                left: tempDontKnowLinkLeft,
                top: tempDontKnowLinkTop + TEMP_DONT_KNOW_LINK_HEIGHT - TEMP_INTERNAL_WALL_THICKNESS,
                width: tempDontKnowLinkWidth,
                height: TEMP_INTERNAL_WALL_THICKNESS,
                  background: "#333",
                    zIndex: 55,
                ...
                <div
                  style={{
                    position: "absolute",
                    left: tempDuBottomNightManDoorLeft,
                    top:
                      tempDontKnowLinkTop +
                      TEMP_DONT_KNOW_LINK_HEIGHT -
                      TEMP_INTERNAL_WALL_THICKNESS -
                      1,
                    width: TEMP_DU_BOTTOM_NIGHT_MAN_DOOR_WIDTH,
                ...
                <div
                  style={{
                    position: "absolute",
                    left: tempDuBottomNightManDoorLeft,
                    top: tempDontKnowLinkTop + TEMP_DONT_KNOW_LINK_HEIGHT - 3,
                    width: 2,
                ...
                <div
                  style={{
                    position: "absolute",
                    left:
                      tempDuBottomNightManDoorLeft +
                      TEMP_DU_BOTTOM_NIGHT_MAN_DOOR_WIDTH -
                      2,
                    top: tempDontKnowLinkTop + TEMP_DONT_KNOW_LINK_HEIGHT - 3,
                    width: 2,
                    height: TEMP_INTERNAL_WALL_THICKNESS + 4,
                ...
                <div
                  style={{
                    position: "absolute",
                    left:
                    tempDontKnowLinkLeft +
                    tempDontKnowLinkWidth -
                    TEMP_DU_BOTTOM_STAIR_GAP_WIDTH,
                  top:
                    tempDontKnowLinkTop +
                    TEMP_DONT_KNOW_LINK_HEIGHT -
                    TEMP_INTERNAL_WALL_THICKNESS -
                    1,
                  width: TEMP_DU_BOTTOM_STAIR_GAP_WIDTH,
                  height: TEMP_INTERNAL_WALL_THICKNESS + 2,
                  background: "#FDE68A",
                  zIndex: 57,
                  pointerEvents: "none",
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuLowerRoom1Left,
                  top: tempDuLowerRoomsTop,
                  width: TEMP_DU_LOWER_ROOM1_WIDTH,
                  height: tempDuLowerRoomsHeight,
                  background: "#D9D9D9",
                  zIndex: 58,
                  pointerEvents: "none",
                }}
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuLowerRoom2Left,
                  top: tempDuLowerRoomsTop,
                  width: tempDuLowerRoom2Width,
                  height: tempDuLowerRoomsHeight,
                  background: "#D9D9D9",
                  zIndex: 58,
                  pointerEvents: "none",
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuLowerRoom3Left,
                  top: tempDuLowerRoomsTop,
                  width: TEMP_DU_LOWER_ROOM3_WIDTH,
                  height: tempDuLowerRoomsHeight,
                  background: "#D9D9D9",
                  zIndex: 58,
                  pointerEvents: "none",
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuLowerRoom4Left,
                  top: tempDuLowerRoomsTop,
                  width: tempDuLowerRoom4Width,
                  height: tempDuLowerRoomsHeight,
                  background: "#D9D9D9",
                  zIndex: 58,
                  pointerEvents: "none",
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuBottomLowerRectLeft,
                  top: tempDuBottomLowerRectTop,
                  width: TEMP_DU_BOTTOM_LOWER_RECT_WIDTH,
                  height: TEMP_DU_BOTTOM_LOWER_RECT_HEIGHT,
                  background: "#FFFFFF",
                  zIndex: 54,
                  pointerEvents: "none",
                }}
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuBottomLowerFillLeft,
                  top: tempDuBottomLowerFillTop,
                  width: tempDuBottomLowerFillWidth,
                  height: tempDuBottomLowerFillHeight,
                  background: "#60A5FA",
              ...
              <div
                style={{
                  ...
                  top: tempDuBottomLowerRectTop,
                  width: TEMP_INTERNAL_WALL_THICKNESS,
                  height: tempDuBottomLowerRectLeftWallHeight,
                  background: "#333",
                  zIndex: 55,
                  pointerEvents: "none",
                }}
              ...
              <div
                style={{
                  ...
                  left:
                    tempDuBottomLowerRectLeft +
                    TEMP_DU_BOTTOM_LOWER_RECT_WIDTH -
                    TEMP_INTERNAL_WALL_THICKNESS,
                  top: tempDuBottomLowerRectTop,
                  width: TEMP_INTERNAL_WALL_THICKNESS,
                  height: TEMP_DU_BOTTOM_LOWER_RECT_HEIGHT,
              ...
              <div
                style={{
                  position: "absolute",
                  left: tempDuBottomLowerRectLineStartLeft,
                  top: tempDuBottomLowerRectLineStartTop,
                  width: tempDuBottomLowerRectLineLength,
                  height: TEMP_INTERNAL_WALL_THICKNESS,
                  background: "#333",
                  transformOrigin: "left center",
                  transform: `rotate(${tempDuBottomLowerRectLineAngle}rad)`,
                  zIndex: 55,
                  pointerEvents: "none",
                }}
              />
              <div
                style={{
                  position: "absolute",
                  left: tempDrugUnitRoomLeft,
                top: tempDrugUnitRoomTop,
                width: TEMP_DRUG_UNIT_ROOM_WIDTH,
                height: TEMP_DRUG_UNIT_ROOM_HEIGHT,
                background: "#D9D9D9",
                zIndex: 54,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempDontKnowLabelLeft,
                top: tempDontKnowLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitRoomLeft,
                top: tempDrugUnitRoomDoorTop,
                width: TEMP_INTERNAL_WALL_THICKNESS,
                height: TEMP_DRUG_UNIT_ROOM_DOOR_WIDTH,
                background: "#D9D9D9",
                zIndex: 56,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitRoomLeft,
                top: tempDrugUnitRoomDoorTop + TEMP_DRUG_UNIT_ROOM_DOOR_WIDTH,
                width: TEMP_INTERNAL_WALL_THICKNESS,
                height:
                  tempDrugUnitRoomTop +
                  TEMP_DRUG_UNIT_ROOM_HEIGHT -
                  (tempDrugUnitRoomDoorTop + TEMP_DRUG_UNIT_ROOM_DOOR_WIDTH),
                background: "#333",
                zIndex: 55,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitRoomLeft - 2,
                top: tempDrugUnitRoomDoorTop - 2,
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitRoomLeft + TEMP_DRUG_UNIT_ROOM_WIDTH - TEMP_INTERNAL_WALL_THICKNESS,
                top: tempDrugUnitRoomTop,
                width: TEMP_INTERNAL_WALL_THICKNESS,
                height: TEMP_DRUG_UNIT_ROOM_HEIGHT,
                background: "#333",
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitRoomLeft,
                top: tempDrugUnitRoomTop + TEMP_DRUG_UNIT_ROOM_HEIGHT - TEMP_INTERNAL_WALL_THICKNESS,
                width: TEMP_DRUG_UNIT_ROOM_WIDTH,
                height: TEMP_INTERNAL_WALL_THICKNESS,
                background: "#333",
                zIndex: 55,
            ...
            <div
              style={{
                position: "absolute",
                left: tempDrugUnitRoomMidLineLeft,
                top: tempDrugUnitRoomMidLineTop,
                width: tempDrugUnitRoomMidLineWidth,
                height: TEMP_INTERNAL_WALL_THICKNESS,
                background: "#333",
                zIndex: 55,
                pointerEvents: "none",
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminSideRectLeft,
                top: tempAdminSideRectTop + tempAdminSideRectHeight - TEMP_WALL_THICKNESS,
                width: TEMP_ADMIN_SIDE_RECT_WIDTH,
                height: TEMP_WALL_THICKNESS,
                background: "#EF4444",
                zIndex: 55,
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminUpperBlockLeft,
                top: tempAdminUpperBlockTop,
                width: TEMP_TOP_HORIZONTAL.width,
                height: tempAdminUpperBlockHeight,
                background: "#D9D9D9",
                zIndex: 54,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempElcWingLabelLeft,
                top: tempElcWingLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminUpperBlockLeft + TEMP_TOP_HORIZONTAL.width - TEMP_WALL_THICKNESS,
                top: tempAdminUpperBlockTop,
                width: TEMP_WALL_THICKNESS,
                height: tempAdminUpperBlockHeight,
                background: "#EF4444",
            ...
            <div
              style={{
                position: "absolute",
                left: tempElcDrugJoinLeft,
                top: tempElcDrugJoinTop + TEMP_WALL_THICKNESS,
                width: TEMP_WALL_THICKNESS,
                height: tempElcDrugJoinHeight - TEMP_WALL_THICKNESS * 2,
                background: "#D9D9D9",
                zIndex: 56,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminUpperBlockLeft,
                top: tempAdminUpperBlockTop + tempAdminUpperBlockHeight - TEMP_WALL_THICKNESS,
                width: TEMP_TOP_HORIZONTAL.width,
                height: TEMP_WALL_THICKNESS,
                background: "#EF4444",
                zIndex: 55,
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminUpperBlockLeft + TEMP_WALL_THICKNESS,
                top: tempElcInternalWallTop,
                width: TEMP_TOP_HORIZONTAL.width - TEMP_WALL_THICKNESS * 2,
                height: TEMP_INTERNAL_WALL_THICKNESS,
                background: "#111",
                zIndex: 55,
                pointerEvents: "none",
            ...
            <div
              style={{
                position: "absolute",
                left: tempElcInternalVerticalLeft,
                top: tempElcInternalVerticalTop,
                width: TEMP_INTERNAL_WALL_THICKNESS,
                height: tempElcInternalVerticalHeight,
                background: "#111",
                zIndex: 55,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingLowerFillLeft,
                top: tempAdminMedicalWingLowerFillTop,
                width: tempAdminMedicalWingLowerFillWidth,
                height: tempAdminMedicalWingLowerFillHeight,
                background: "#D9D9D9",
                zIndex: 54,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingTopFillLeft,
                top: tempAdminMedicalWingTopFillTop,
                width: tempAdminMedicalWingTopFillWidth,
                height: tempAdminMedicalWingTopFillHeight,
                background: "#D9D9D9",
                zIndex: 54,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingUpperFillLeft,
                top: tempAdminMedicalWingUpperFillTop,
                width: tempAdminMedicalWingUpperFillWidth,
                height: tempAdminMedicalWingUpperFillHeight,
                background: "#D9D9D9",
                zIndex: 56,
                pointerEvents: "none",
              }}
            ...
            <div
              style={{
                position: "absolute",
                left: tempSurgeryStoreLabelLeft,
                top: tempSurgeryStoreLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
            ...
            <div
              style={{
                position: "absolute",
                left: tempDoctorwWaitingRoomLabelLeft,
                top: tempDoctorwWaitingRoomLabelTop,
                padding: "3px 8px",
                background: "rgba(255,255,255,0.96)",
                border: "1px solid rgba(239,68,68,0.45)",
                borderRadius: 6,
            ...
            <div
              style={{
                position: "absolute",
                left: tempTopHorizontalLeft + TEMP_WALL_THICKNESS,
                top: tempAdminMedicalWingUpperWallTop,
                width:
                  tempAdminMedicalWingDoorLeft - (tempTopHorizontalLeft + TEMP_WALL_THICKNESS),
                height: TEMP_INTERNAL_WALL_THICKNESS,
                background: "#111",
                zIndex: 55,
                pointerEvents: "none",
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingDoorLeft - 1,
                top: tempAdminMedicalWingUpperWallTop - 1,
                width: TEMP_ADMIN_MEDICAL_WING_DOOR_WIDTH + 2,
                height: TEMP_INTERNAL_WALL_THICKNESS + 2,
                background: "#D9D9D9",
                zIndex: 56,
                pointerEvents: "none",
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingDoorLeft - 2,
                top: tempAdminMedicalWingUpperWallTop - 2,
                width: 2,
                height: TEMP_INTERNAL_WALL_THICKNESS + 4,
                background: "#111",
                zIndex: 57,
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingDoorLeft + TEMP_ADMIN_MEDICAL_WING_DOOR_WIDTH,
                top: tempAdminMedicalWingUpperWallTop - 2,
                width: 2,
                height: TEMP_INTERNAL_WALL_THICKNESS + 4,
                background: "#111",
            ...
            <div
              style={{
                ...
                width:
                  tempTopHorizontalLeft +
                  TEMP_TOP_HORIZONTAL.width -
                  TEMP_WALL_THICKNESS -
                  (tempAdminMedicalWingDoorLeft + TEMP_ADMIN_MEDICAL_WING_DOOR_WIDTH),
                height: TEMP_INTERNAL_WALL_THICKNESS,
                background: "#111",
                zIndex: 55,
                pointerEvents: "none",
            ...
            <div
              style={{
                position: "absolute",
                left: tempTopHorizontalLeft + TEMP_WALL_THICKNESS,
                top: tempAdminMedicalWingInnerWallTop,
                width:
                  tempAdminMedicalWingDoorLeft - (tempTopHorizontalLeft + TEMP_WALL_THICKNESS),
                height: TEMP_INTERNAL_WALL_THICKNESS,
                background: "#111",
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingDoorLeft - 1,
                top: tempAdminMedicalWingInnerWallTop - 1,
                width: TEMP_ADMIN_MEDICAL_WING_DOOR_WIDTH + 2,
                height: TEMP_INTERNAL_WALL_THICKNESS + 2,
                background: "#D9D9D9",
                zIndex: 56,
            ...
            <div
              style={{
                position: "absolute",
                left: tempAdminMedicalWingDoorLeft - 2,
                top: tempAdminMedicalWingInnerWallTop - 2,
                width: 2,
                height: TEMP_INTERNAL_WALL_THICKNESS + 4,
                background: "#111",
                zIndex: 57,
